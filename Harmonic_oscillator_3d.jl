...
#function Lagrangian(x0, x1, m, omega)
    ...
    S = 0.5 * m * (norm(x1 - x0)^2 + norm(x2 - x0)^2) + (0.5 * m * omega^2 * norm(x0)^2) 
    return S
end  
...
function Thermodynamic_energy(x)
    ...
    """
    n_beads = N
    x_circ = x # Create a circular array to avoid B.C. problems
    ...
    for i in 1:n_beads
        ...
        K += (0.5 * m * (norm(x_circ[i+1, :] - x_circ[i, :])^2)) 
        V += (0.5 * m * omega^2 * norm(x_circ[i, :])^2) 
    end
    V = V/n_beads # <V>
    K = K/n_beads # <K>
    H = 3/2/delta_tau - (K - V) # Definition of thermodynamic estimator
    return H
end
...
function Virial_energy(x)
    ...
    K = 0
    V = 0
    COM = zeros(dimension) # Centre of Mass of the beads
...
function Metropolis_Update(x, h, record_energy = false)
    ...
    for t in random_ints
        ...
        rand_change = 2 * h .* (rand(dimension) .- 0.5)

        new_x = x[t, :] + rand_change # Change all 3 coordinates
        ...
        S_old = Lagrangian(x[t, :], x[t+1, :], x[t-1, :], m, omega)
        S_new = Lagrangian(new_x, x[t+1, :], x[t-1, :], m, omega)
...
function Wavefunction_Test(x, h)
    ...
    path_collection, H_array_virial, H_array_thermo = Collect_Thermalised_Paths(x, h, 100, 500, 20)
...
for i in eachindex(Beta_arr)
    ...
    =#
    global N = floor(Int, beta / delta_tau) 
    global id_rate = 0.5 #ideal rate
    global init_pos = CircularArray(zeros(N, dimension))
...
Avg_E_theoretical = 3/2*hbar*omega * (1 .+ exp.(-hbar*omega*Beta_arr))./(1 .- exp.(-hbar*omega*Beta_arr))
